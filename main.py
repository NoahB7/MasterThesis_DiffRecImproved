...
parser.add_argument('--dataset', type=str, default='ml-1m_custom', help='choose the dataset')
parser.add_argument('--data_path', type=str, default='./datasets/ml-1m_clean/', help='load data path') # CLEAN DATA IS ONLY MOVIES THE USER RATED A 5, NOISY CONTAINS UNDER 5
...
parser.add_argument('--emb_size', type=int, default=100, help='timestep embedding size')
parser.add_argument('--p_uncond', type=float, default=0.2, help='probability when using classifier-free guidance to generate without guidance') # added
...
train_y_data = train_data[int(np.floor(train_data.shape[0]*.8))+1:] # more training data, used as labels for the 20% validation
valid_y_data = valid_y_data[int(np.floor(valid_y_data.shape[0]*.8))+1:] # these all have to be the same size for some reason

train_data = train_data[:int(np.floor(train_data.shape[0]*.8))] # actual training data
...
valid_dataset = data_utils.DataDiffusion(torch.FloatTensor(valid_y_data.A)) # added
...
# valid_loader, valid_y_data, train_y_data, (valid and train are the same here...?)
...
def evaluate(data_loader, data_te, mask_his, topN):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(data_loader):
            ...
            prediction[his_data.nonzero()] = -np.inf # setting predictions to -infinity where we already know users rated a 5

            _, indices = torch.topk(prediction, 100) # getting the 100 largest values from prediction
            indices = indices.cpu().numpy().tolist()
            predict_items.extend(indices)

    print(predict_items[2])
    print(target_items[2])
...
for epoch in range(1, args.epochs+1):
    ...
    if epoch % 5 == 0:
        ...
        valid_results = evaluate(valid_loader, valid_y_data, train_y_data, eval(args.topN))
        evaluate_utils.print_results(None, valid_results, None)
        
        if valid_results[2][3] > best_nDCG: # recall@20 as selection
            best_nDCG, best_epoch = valid_results[2][3], epoch
            ...
            torch.save(model, '{}{}_{}P@10_{}R@10_{}N@10_{}MRR.pth'.format(args.save_path, args.dataset, valid_results[0][2], valid_results[1][2], valid_results[2][2], valid_results[3][2]))
            prev = '{}{}_{}P@10_{}R@10_{}N@10_{}MRR.pth'.format(args.save_path, args.dataset, valid_results[0][2], valid_results[1][2], valid_results[2][2], valid_results[3][2])
